...
actor target = actor("", t_stat(t_value(50), t_value(7), t_value(4), t_element(3, 3), t_element(3, 3)), 0);

static void	static_save_actor(void *value)
{
	save_actor(&target);
}
...
void		menu_player_editor(void *value)
{
	...
	menu.add_list_box(list_box("Heroes selected : ", " - Create a new char - ", menu.get_win_unit() * t_f_vect(0.5, 0.5), menu.get_win_unit() * t_f_vect(5, 0.5), color, color2));
	(*(menu.get_list_box()))[0].actualize_line(list_files(ACTOR_PATH, ACTOR_EXT));

	menu.add_text_box(text_box("Enter your name", target.get_name(), DARK_GREY, menu.get_win_unit().x / 5, menu.get_win_unit() * t_f_vect(0.5, 1.2), menu.get_win_unit() * t_f_vect(5, 0.5), color, color2));
	...
	menu.add_text_button(text_button("Save character", DARK_GREEN, 35, menu.get_win_unit() * t_f_vect(8, 7), menu.get_win_unit() * t_f_vect(4, 1), color, color2, static_save_actor));
	...
	while (quit == false)
	{
		...
		if (SDL_PollEvent(&(event)) == 1)
		{
			...
			else if (event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_BACKSPACE && menu.get_selected_text_box() != NULL)
			{
				...
				target.set_name(menu.get_selected_text_box()->get_text());
			}
